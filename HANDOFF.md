...
## ⭐ HANDOFF mới nhất (2026-08-18) — Châm ngôn "Crypto and AI" (gỡ tiếng Việt khỏi CV)

**Câu định vị chính thức, dùng nguyên văn ở mọi nơi: `Crypto and AI`.** (Cùng ngày user rút gọn từ bản đầu `Sharing POVs on Crypto + AI` — ảnh `og.png` lên `?v=5`, cỡ chữ tiêu đề 64px → **80px** cho cân vì câu ngắn hơn nhiều.) Niềm tin đứng sau: *AI là tương lai, crypto là tiền tệ của tương lai* (câu này đi kèm ở meta/og/JSON-LD, không thay thế châm ngôn).
...
- `.hero-tagline` (tab CV) → `Crypto and AI` (bỏ dấu chấm cuối — là châm ngôn, không phải câu văn)
