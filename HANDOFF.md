...
- **LUẬT — thang rank khai 1 chỗ duy nhất** trong `index.html` (mục "AIRDROP — Work to Earn"): `WTE_GROUPS` (thứ tự nhóm) · `WTE_RANKS` (rank chọn tay được = `WTE_GROUPS` bỏ `INCOME`) · `RANK_LABEL` (chữ trên badge) · `GROUP_LABEL` (nhãn tiêu đề nhóm). Dropdown Rank của form admin **đổ bằng JS từ `WTE_RANKS`**, không hardcode `<option>` trong HTML, để form không bao giờ lệch thang. Thêm/đổi nhóm = sửa 4 hằng số này + thêm `.rank-*`/`.wte-card--*` trong CSS + thêm nút cùng `data-rank` vào `#rank-filter`
