...
- 2026-07-19: **Cột Ticker (bảng Valuation): căn giữa theo KHỐI** — trước đó căn trái toàn cột; giờ mỗi ticker bọc trong `span.tk` rộng bằng nhau (`min-width: var(--tk-w)` = bề rộng ticker dài nhất, JS đo bằng probe ẩn trong `renderTable` vì bảng có thể đang `display:none`), cell `text-align:center` → mọi ticker thẳng mép trái với nhau, cả khối nằm giữa cột. Modal TGE clone tbody nên tự ăn theo (`--tk-w` set trên `:root`).
